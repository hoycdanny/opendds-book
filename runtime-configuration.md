...
| DCPSChunks=n                                                                                                    | 當`RESOURCE_LIMITS`Qos值為無限時，資料讀寫快取分配之可設定主幹數量會得到預先分配之快取。當所有主幹都被分配出去時，OpenDDS將會由heap再分配。                                                                                                                           | 20                                                      |
| DCPSChunkAssociationMultiplier=n                                                                                | DCPSChunks的多工器或是藉由resource_limits.max_samples數值判斷預分配之淺複製主幹總數。將這個值設的比連接數量還大以防止預分配主幹過少。一個取樣寫入到多個資料讀取者不會執行多次複製而是會執行多次淺複製。這個動作佔用空間並不大，所以不需要把這個值設的太接近連結數量。 | 10                                                      |
